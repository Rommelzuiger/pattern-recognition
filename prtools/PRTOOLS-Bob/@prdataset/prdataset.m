...
function varargout = prdataset(a,labels,varargin)
	...
	if isempty(PRTOOLS_START)
      ...
      %                 ' (' z.Date ').'])
      disp([newline '   Welcome to PRTools5. It is not fully compatible with PRTools4.'])
      ...
      disp([newline '   Type ''prnews'' to open your browser for PRTools news'])
			disp([prtools_mod newline]) % Display message of the day
		end
	end
	...
	elseif isa(a,'prdatafile') % convert from datafile class
		...
		while next0 > 0
			try
				[c,next1,J] = readdatafile(a,next0,0,0);
      catch
				ss = lasterror;
