...
function varargout = crossval(varargin)

  persistent crossval_warning
  if isempty(crossval_warning)
    warning('Use of ''crossval'' for PRTools variables is deprecated. Use ''prcrossval'' instead');
    crossval_warning = 1;
  end

  varargout = cell(1,nargout);
  [varargout{:}] = prcrossval(varargin{:});
